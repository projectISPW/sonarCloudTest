...
public class UtenteUsr extends SalvaUtente {
    ...
    public UtenteSQL sql=new UtenteSQL();
    public void  dataHomeUsr () {
        try{
            rst = sql.getUserData(userid);
            while(rst.next()){
    ...
    public void queryResult() {
        try{
            ...
            for(Integer id :calculated) sql.setCalculated(userid,id);
            if(edited){
                System.err.println("parametri aggiornati emp"+emp+"hum"+hum+"pos"+pos);
                sql.setParams(userid,new int[]{emp,hum,pos});
            }
        }catch (SQLException throwables) {
            throwables.printStackTrace();
        }
